...
public class Button {
	...
	private int x, y, w, h, sw, sh, s;
	private String text;

	public Button(int midX, int midY, String text, int textSize) {
		s = textSize;
		this.text = text;
		GameRenderer r = GameController.get().renderer;
		sw = s * r.getStringWidth(text);
		sh = s * r.getStringHeight(text);
		w = sw + 2 * s * PADDING_W;
		h = sh + 2 * s * PADDING_H;
		x = midX - w / 2;
		y = midY - h / 2;
	...
	public Button(int midX, int midY, int w, String text, int textSize) {
		s = textSize;
		this.w = w;
		this.text = text;
		GameRenderer r = GameController.get().renderer;
	...
	public void draw(GameRenderer r){
		...
		g.drawRect(x, y, w, h);
		r.drawStringOnScreen(text, x+(w-sw)/2, y+(h-sh)/2, GameRenderer.COLOR_TEXT_NORMAL, s);
	}
